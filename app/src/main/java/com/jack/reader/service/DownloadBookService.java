 ...
 */
package com.jack.reader.service;
...
import com.jack.reader.api.support.Logger;
import com.jack.reader.api.support.LoggingInterceptor;
...
import com.jack.reader.utils.NetworkUtils;
...
public class DownloadBookService extends Service {
                ...
                for (int i = start; i <= end && i <= list.size(); i++) {
                    ...
                    if (!downloadQueue.isFinish && !downloadQueue.isCancel) {
                        ...
                        if (CacheManager.getInstance().getChapterFile(bookId, i) == null) {
                            ...
                            int ret = download(downloadQueue.lat, downloadQueue.lng, url, bookId, chapters.title, i, list.size());
                            if (ret != 1) {
                                failureCount++;
                            }
                        } else {
    ...
    private int download(double lat, double lng, String url, final String bookId, final String title, final int chapter, final int chapterSize) {
        final int[] result = {-1};
        MultipartBody.Builder builder = AppUtils.getDefaultBody(getApplicationContext());
        builder.addFormDataPart("reg_lat", String.valueOf(lat));
        builder.addFormDataPart("reg_lng", String.valueOf(lng));
        builder.addFormDataPart("secr", url);
        RequestBody build = builder.build();
        Subscription rxSubscription = bookApi.getChapterRead(build).subscribeOn(Schedulers.io())
                                ...
                                if (data1 != null) {
                                    if (StringUtils.isNotEmpty(data1.getContent())) {
                                        post(new DownloadProgress(bookId, String.format(
                                                ...
                                                true));
                                        CacheManager.getInstance().saveChapterFile(bookId, chapter, new ChapterRead.Chapter(data1.getContent()));
                                        result[0] = 1;
                                    } else {
